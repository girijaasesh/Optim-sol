...
<div class="page-wrap">
  <div class="page-header" style="margin-bottom:1.5rem">
    <div>
      <h1 style="font-family:'Lora',serif;font-size:1.75rem;font-weight:700">Coupons</h1>
      <p style="color:var(--ink-mid);font-size:0.875rem">Promotional and discount codes</p>
    </div>
  </div>
  ...
  <div class="page-header">
    <div><h1>Coupons</h1><p>Promotional and discount codes</p></div>
    <button class="btn btn-primary" (click)="showCreate.set(true)">+ New Coupon</button>
  </div>
  <div class="table-card" *ngIf="!loading()">
    <table class="data-table">
      <thead><tr><th>Code</th><th>Type</th><th>Value</th><th>Used</th><th>Limit</th><th>Expires</th><th>Status</th></tr></thead>
      <tbody>
        <tr *ngFor="let c of coupons()">
          <td><code class="ref-code">{{ c.code }}</code></td>
          <td>{{ c.discountType }}</td>
          <td><strong>{{ c.discountType === 'PERCENTAGE' ? c.discountValue + '%' : (c.discountValue | currency) }}</strong></td>
          <td>{{ c.usageCount }}</td>
          <td>{{ c.usageLimit || '∞' }}</td>
          <td>{{ c.expiryDate | date || 'Never' }}</td>
          <td><span class="badge" [class]="c.active ? 'badge-green' : 'badge-grey'">{{ c.active ? 'Active' : 'Inactive' }}</span></td>
        </tr>
      </tbody>
    </table>
    <div *ngIf="coupons().length === 0" class="empty-state"><div class="empty-icon">🏷</div><h3>No coupons yet</h3></div>
  </div>
  <div class="modal-backdrop" *ngIf="showCreate()" (click)="showCreate.set(false)">
    <div class="modal-box" (click)="$event.stopPropagation()">
      <div class="modal-header"><h3>Create Coupon</h3><button class="modal-close" (click)="showCreate.set(false)">✕</button></div>
      <div class="modal-body">
        <div *ngIf="msg()" class="alert" [class]="msgType()==='ok'?'alert-ok':'alert-err'">{{ msg() }}</div>
        <form [formGroup]="form" class="form-grid">
          <div class="form-group"><label class="form-label">Code *</label><input formControlName="code" class="form-control" placeholder="EARLYBIRD26"></div>
          <div class="form-group"><label class="form-label">Type *</label><select formControlName="discountType" class="form-control"><option value="PERCENTAGE">Percentage %</option><option value="FIXED_AMOUNT">Fixed Amount $</option></select></div>
          <div class="form-group"><label class="form-label">Value *</label><input formControlName="discountValue" type="number" class="form-control" placeholder="20"></div>
          <div class="form-group"><label class="form-label">Usage Limit</label><input formControlName="usageLimit" type="number" class="form-control" placeholder="Leave blank for unlimited"></div>
          <div class="form-group"><label class="form-label">Expiry Date</label><input formControlName="expiryDate" type="date" class="form-control"></div>
          <div class="form-group"><label class="form-label">Course Type (blank = all)</label><select formControlName="applicableCourseType" class="form-control"><option value="">All Courses</option><option value="SAFE_AGILIST">SAFe Agilist</option><option value="CORPORATE_WORKSHOP">Corporate</option></select></div>
        </form>
      </div>
      <div class="modal-footer"><button class="btn btn-outline" (click)="showCreate.set(false)">Cancel</button><button class="btn btn-primary" (click)="create()" [disabled]="saving()">{{ saving() ? 'Creating…' : 'Create Coupon' }}</button></div>
    </div>
  </div>
</div>
  `,
