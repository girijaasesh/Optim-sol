...
<div class="page-wrap">
  <div class="page-header" style="margin-bottom:1.5rem">
    <div>
      <h1 style="font-family:'Lora',serif;font-size:1.75rem;font-weight:700">Waitlist</h1>
      <p style="color:var(--ink-mid);font-size:0.875rem">Manage course waitlists</p>
    </div>
  </div>
  ...
  <div class="info-banner">
    <span>💡</span>
    <div>When a seat opens (cancellation/refund), the first person on the waitlist is automatically notified by email with a 48-hour window to register.</div>
  </div>
  <div class="table-card" *ngIf="!loading()">
    <table class="data-table">
      <thead><tr><th>#</th><th>Participant</th><th>Course</th><th>Joined</th><th>Status</th><th>Notified</th></tr></thead>
      <tbody>
        <tr *ngFor="let w of waitlist()">
          <td><strong>#{{ w.position }}</strong></td>
          <td><div class="td-title">{{ w.user?.fullName }}</div><div class="td-sub">{{ w.user?.email }}</div></td>
          <td>{{ w.course?.title }}</td>
          <td>{{ w.joinedAt | date:'d MMM yy' }}</td>
          <td><span class="badge" [class]="wBadge(w.status)">{{ w.status }}</span></td>
          <td>{{ w.notifiedAt ? (w.notifiedAt | date:'d MMM HH:mm') : '—' }}</td>
        </tr>
      </tbody>
    </table>
    <div *ngIf="waitlist().length === 0" class="empty-state"><div class="empty-icon">⏳</div><h3>No one on the waitlist</h3></div>
  </div>
</div>
  `,
  styles: [`
