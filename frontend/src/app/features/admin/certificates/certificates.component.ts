...
<div class="page-wrap">
  <div class="page-header" style="margin-bottom:1.5rem">
    <div>
      <h1 style="font-family:'Lora',serif;font-size:1.75rem;font-weight:700">Certificates</h1>
      <p style="color:var(--ink-mid);font-size:0.875rem">Issue and manage SAFe certification records</p>
    </div>
  </div>
  ...
  <div class="table-card" *ngIf="!loading()">
    <table class="data-table">
      <thead><tr><th>Cert ID</th><th>Participant</th><th>Certification</th><th>Issue Date</th><th>Status</th><th>Actions</th></tr></thead>
      <tbody>
        <tr *ngFor="let c of certs()">
          <td><code class="ref-code">{{ c.certificateId }}</code></td>
          <td>{{ c.participantName }}</td>
          <td>{{ c.certificationName }}</td>
          <td>{{ c.issueDate | date }}</td>
          <td><span class="badge badge-green">{{ c.status }}</span></td>
          <td><button class="btn-icon-sm" (click)="download(c)">⬇ PDF</button></td>
        </tr>
      </tbody>
    </table>
    <div *ngIf="certs().length === 0" class="empty-state"><div class="empty-icon">🏆</div><h3>No certificates issued yet</h3><p>Confirm registrations and mark courses complete to issue certificates</p></div>
  </div>
  <div class="quick-issue card" style="margin-top:1.5rem">
    <div class="card-header"><h3>Quick Issue Certificate</h3></div>
    <div style="padding:1.5rem;display:flex;gap:1rem;align-items:flex-end">
      <div class="form-group" style="flex:1;margin:0">
        <label class="form-label">Registration ID</label>
        <input class="form-control" [(ngModel)]="issueRegId" type="number" placeholder="e.g. 42">
      </div>
      <button class="btn btn-primary" (click)="issueCert()" [disabled]="issuing()">{{ issuing() ? 'Issuing…' : 'Issue Certificate' }}</button>
    </div>
    <div *ngIf="issueMsg()" class="alert" style="margin:0 1.5rem 1.5rem" [class]="issueMsgType()==='ok'?'alert-ok':'alert-err'">{{ issueMsg() }}</div>
  </div>
</div>
  `,
  styles: [`
