...
<div class="page-wrap">
  <div class="page-header" style="margin-bottom:1.5rem">
    <div>
      <h1 style="font-family:'Lora',serif;font-size:1.75rem;font-weight:700">Schedule</h1>
      <p style="color:var(--ink-mid);font-size:0.875rem">Upcoming course calendar</p>
    </div>
  </div>
  ...
  <div class="table-card" *ngIf="!loading()">
    <table class="data-table">
      <thead><tr><th>Course</th><th>Type</th><th>Start</th><th>End</th><th>Format</th><th>Seats</th><th>Status</th></tr></thead>
      <tbody>
        <tr *ngFor="let c of courses()">
          <td><div class="td-title">{{ c.title }}</div></td>
          <td><span class="badge badge-teal">{{ formatType(c.certificationType) }}</span></td>
          <td>{{ c.startDate | date:'d MMM yyyy' }}</td>
          <td>{{ c.endDate | date:'d MMM yyyy' }}</td>
          <td>{{ c.format === 'VIRTUAL' ? '💻 Virtual' : '🏢 In-Person' }}</td>
          <td><span [class.text-red]="c.seatsRemaining === 0" [class.text-amber]="c.seatsRemaining <= 5 && c.seatsRemaining > 0">{{ c.seatsRemaining }}/{{ c.maxSeats }}</span></td>
          <td><span class="badge" [class]="c.soldOut ? 'badge-red' : 'badge-green'">{{ c.soldOut ? 'Sold Out' : 'Open' }}</span></td>
        </tr>
      </tbody>
    </table>
    <div *ngIf="courses().length === 0" class="empty-state"><div class="empty-icon">📅</div><h3>No upcoming sessions</h3></div>
  </div>
</div>
  `,
  styles: [`
